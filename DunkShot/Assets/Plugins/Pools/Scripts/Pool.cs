...
namespace Redcode.Pools
{
    ...
    public static class Pool
    {
        ...
        /// <returns></returns>
        public static Pool<T> Create<T>(T source, Func<T> createFunc, int count = 0, Transform container = null) where T : Component => Pool<T>.Create(source, createFunc, count, container);
    }
    ...
    public class Pool<T> : IPool<T> where T : Component
    {
        ...
        internal static Pool<T> Create(T source, Func<T> createFunc, int count, Transform container)
        {
            var pool = new Pool<T>
            {
                _source = source,
